...
# 配置参数
CONFIG_FILE="configs/4_shot/foodx251_cosine_5e_6_ls.yaml"
...
# 启动多GPU分布式训练
echo "🔥 启动多GPU分布式训练..."
nohup deepspeed --master_port=29501 --num_gpus=$NUM_GPUS \
    training/complete_train.py \
    --config $CONFIG_FILE \
    --deepspeed_config $DEEPSPEED_CONFIG > /llm_reco/dehua/code/qwen_classify/logs/foodx251_ls_5e_6_4shot.log 2>&1
