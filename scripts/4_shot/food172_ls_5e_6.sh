...
# 配置参数
CONFIG_FILE="configs/4_shot/food172_cosine_5e_6_ls.yaml"
...
nohup deepspeed --master_port=$MASTER_PORT --num_gpus=$NUM_GPUS \
    training/train.py \
    --config $CONFIG_FILE \
    --deepspeed_config $DEEPSPEED_CONFIG > /llm_reco/dehua/code/qwen_classify/logs/food172_ls_5e_6_4shot.log 2>&1
